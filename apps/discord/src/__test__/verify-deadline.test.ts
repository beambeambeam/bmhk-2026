...
describe(formatVerifyClosedMessage, () => {
  it("points participants at the support channel when configured", () => {
    expect(formatVerifyClosedMessage("123")).toBe(
      "หมดช่วงเวลาการยืนยันตัวตนแล้ว (กฎกติกาการแข่งขันที่ใช้ในการแข่งขันรอบคัดเลือก ข้อที่ 10.2) หากนี่คือข้อผิดพลาด สามารถติดต่อทีมงานได้ที่ช่อง <#123>",
    );
  });

  it("omits the channel when none is configured", () => {
    expect(formatVerifyClosedMessage(null)).toBe(
      "หมดช่วงเวลาการยืนยันตัวตนแล้ว (กฎกติกาการแข่งขันที่ใช้ในการแข่งขันรอบคัดเลือก ข้อที่ 10.2) หากนี่คือข้อผิดพลาด สามารถติดต่อทีมงานได้",
    );
  });
});
