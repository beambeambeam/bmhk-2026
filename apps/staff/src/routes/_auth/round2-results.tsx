...
export const Route = createFileRoute("/_auth/round2-results")({
  beforeLoad: ({ context }) => {
  ...
  component: Round2ResultsRoute,
});

function Round2ResultsRoute() {
  const { session } = Route.useRouteContext();
  return <TeamRoundResultsPage actorId={session.data?.user.id} round="ROUND_2" />;
}
