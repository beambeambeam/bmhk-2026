...
/* oxlint-disable unicorn/prefer-number-coercion */
...
function ToastCard({ toast, onDismiss }: { toast: ToastModel; onDismiss: () => void }) {
  ...
  const ratio = total != null && total !== 0 ? Math.min(1, Math.max(0, (loaded ?? 0) / total)) : 0;
...
function motionTokens() {
  if (!tokens) {
    ...
    const base = Number(style.getPropertyValue("--mm-base"));
    tokens = {
...
function useSettle(listRef: React.RefObject<HTMLOListElement | null>) {
  ...
  const sample = (animate: boolean) => {
    ...
    for (const child of [...list.children]) {
      const row = child instanceof HTMLElement ? child : null;
      if (!row) {
        continue;
      }
